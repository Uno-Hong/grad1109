...
<%
		...
		try{
		 ...
		 //String sql = "select uid, upw from mpuser where uid = '" + entid + "'";
			String sqlCount = "select count(*) from mppost";
			...
			int sort = 1;
			String sqlSort = "select pnum from mppost order by step asc";
			rs = stmt.executeQuery(sqlSort);
			
			while(rs.next()){
				int stepnum = rs.getInt(1);
				sqls = "update mppost set step = " + sort + " where pnum = " + stepnum;
		 ...
		 String sql = "select pnum, puid, psub, pcount, left(pupt,10) from mppost where step >=" + start + " and step <= " + end + " order by step asc";
...
<script type="text/javascript">
	function gowriteForm() { //글쓰기버튼 클릭시 글쓰기 폼으로 이동
		location.href="board_writeform.jsp";
	} 
</script>
</head>
<body>
...
<div name="fbform" align="center" id="form">
	...
	%>		
			<tr>
				<td style="text-align: center; font-weight: bold;"><%= pnum %></td>
				<td style="text-align: left;">
					<a href="board_view.jsp?pnum=<%=pnum%>&pg=<%=pg %>"><%= psub%>
					<%
						yea = pupt.substring(0,10);
						if(year.equals(yea)){ %>
							<img src="img/new.jpg">
					<%} %>
				</td>
				<td style="text-align: center;"><%= puid %></td>
				<td style="text-align: center;"><%= pupt %></td>
